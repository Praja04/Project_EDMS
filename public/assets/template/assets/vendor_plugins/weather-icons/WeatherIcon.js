...
var WeatherIcon = (function () {
  ...
  WeatherIcon.prototype = {
    ...
    change: function (icon, mode) {
      ...
      this.setIcon(icon);
      this.setBody(
        mode === WeatherIcon.NIGHT ? WeatherIcon.NIGHT : WeatherIcon.DAY,
      );
      this.build();
      this.draw();
    },
    ...
    addRain: function (type) {
      ...
      while (n--)
        this.particles.push(
          new WeatherIcon.Drop(
            WeatherIcon.particles[type][n],
            speed,
            angle,
            yo,
          ),
    ...
    build: function () {
      ...
      switch (this.icon) {
        case WeatherIcon.SUN:
          this.objects.push(
            this.body == WeatherIcon.DAY
              ? new WeatherIcon.Sun(64, 64, 30)
              : new WeatherIcon.Moon(64, 64, 30),
          );
          break;
        ...
        case WeatherIcon.PARTLYCLOUD:
          ...
          this.objects.push(
            this.body == WeatherIcon.DAY
              ? new WeatherIcon.Sun(64, 54, 30)
              : new WeatherIcon.Moon(64, 54, 30),
          );
          break;
        ...
        case WeatherIcon.LIGHTRAINSUN:
          ...
          this.objects.push(
            this.body == WeatherIcon.DAY
              ? new WeatherIcon.Sun(40, 30, 20)
              : new WeatherIcon.Moon(40, 30, 20),
          );
  ...
  WeatherIcon.Cloud = (function () {
    var Cloud = function (xo, yo, w) {
      ...
      this.cl = new WeatherIcon.Circle(
        new WeatherIcon.Point(center.x - sizeMed.x, center.y),
        w * 0.14,
      );
      this.cr = new WeatherIcon.Circle(
        new WeatherIcon.Point(center.x + sizeMed.x, center.y + w * 0.03),
        w * 0.1,
      );

      this.ca = new WeatherIcon.Circle(
        new WeatherIcon.Point(center.x - sizeMed.x * 0.42, center.y),
        w * 0.35,
      );
      this.cb = new WeatherIcon.Circle(
        new WeatherIcon.Point(center.x + sizeMed.x * 0.45, center.y),
        w * 0.25,
      );
      this.cc = new WeatherIcon.Circle(
        new WeatherIcon.Point(center.x - sizeMed.x * 0.2, center.y),
        w * 0.28,
      );
  ...
  WeatherIcon.Snow = (function () {
    var Snow = function (posIni, speed, angle_, yo) {
      ...
      this.rot = new WeatherIcon.Point(
        Math.cos(this.angle),
        Math.sin(this.angle),
      );
      ...
      draw: function (ctx) {
        ...
        if (this.isFreeze) {
          ...
          ctx.bezierCurveTo(
            ...
            this.po.x,
            this.po.y + 1e-5,
          ); // +1e-5 debug chrome bezierCurveTo bug
        }
        ctx.closePath();
        ctx.stroke();
        ctx.fill();
  ...
  WeatherIcon.add = function (dom, icon, param) {
    ...
    var weatherIcon = new WeatherIcon(
      dom,
      param.stroke === false ? false : true,
      param.shadow === true ? true : false,
    );

    weatherIcon.setIcon(icon);
    weatherIcon.setBody(
      param.mode === WeatherIcon.NIGHT ? WeatherIcon.NIGHT : WeatherIcon.DAY,
    );
    weatherIcon.build();
    weatherIcon.draw();
