(function (venn) {
  ...
  function computeTextCenters(sets, width, height, diagram) {
    ...
    for (var i = 0; i < sets.length; ++i) {
      var sum = sums[i];
      if (sum.count) {
        sets[i].textCenter = { x: sum.x / sum.count, y: sum.y / sum.count };
      } else {
        // no sampled points, possibly completely overlapped (or tiny)
        // use circle centre
        sets[i].textCenter = { x: sets[i].x, y: sets[i].y };
      }
    }
  }
  ...
  venn.drawD3Diagram = function (element, dataset, width, height, parameters) {
    ...
    var svg = element.append("svg").attr("width", width).attr("height", height);
    ...
    return { svg: svg, nodes: nodes, circles: circles, text: text };
  };
  ...
  venn.intersectionArea = function (circles, stats) {
    ...
    if (innerPoints.length > 1) {
      ...
      for (i = 0; i < circles.length; ++i) {
        if (
          venn.distance(circles[i], smallest) >
