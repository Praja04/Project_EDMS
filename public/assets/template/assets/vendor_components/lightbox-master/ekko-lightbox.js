  ...
  class Lightbox {
    ...
    constructor($element, config) {
      ...
      $(this._config.doc.body).append(
        `<div id="${this._modalId}" class="ekko-lightbox modal fade" tabindex="-1" tabindex="-1" role="dialog" aria-hidden="true">${dialog}</div>`,
      );
      ...
      if (this._galleryName) {
        ...
        if (this._config.showArrows && this._$galleryItems.length > 1) {
          this._$lightboxContainer.append(
            `<div class="ekko-lightbox-nav-overlay"><a href="#">${this._config.leftArrow}</a><a href="#">${this._config.rightArrow}</a></div>`,
          );
    ...
    updateNavigation() {
      if (!this._config.wrapping) {
        let $nav = this._$lightboxContainer.find(
    ...
    _detectRemoteType(src, type) {
      ...
      if (
        !type ||
        ["image", "youtube", "vimeo", "instagram", "video", "url"].indexOf(
          type,
        ) < 0
      )
        type = "url";
    ...
    _handle() {
      ...
      let currentRemote =
        this._$element.attr("data-remote") || this._$element.attr("href");
      let currentType = this._detectRemoteType(
        currentRemote,
    ...
    _showYoutubeVideo(remote, $containerForElement) {
      ...
      let query =
        remote.indexOf("&") > 0 ? remote.substr(remote.indexOf("&")) : "";
    ...
    _showInstagramVideo(id, $containerForElement) {
      ...
      $containerForElement.html(
        `<iframe width="${width}" height="${height}" src="${id}embed/" frameborder="0" allowfullscreen></iframe>`,
      );
    ...
    _showVideoIframe(url, width, height, $containerForElement) {
      ...
      $containerForElement.html(
        `<div class="embed-responsive embed-responsive-16by9"><iframe width="${width}" height="${height}" src="${url}" frameborder="0" allowfullscreen class="embed-responsive-item"></iframe></div>`,
      );
      this._resize(width, height);
      this._config.onContentLoaded.call(this);
    ...
    _showHtml5Video(url, $containerForElement) {
      ...
      let height = this._$element.data("height") || width / (560 / 315);
      $containerForElement.html(
        `<div class="embed-responsive embed-responsive-16by9"><video width="${width}" height="${height}" src="${url}" preload="auto" autoplay controls class="embed-responsive-item"></video></div>`,
      );
      this._resize(width, height);
      this._config.onContentLoaded.call(this);
      if (this._$modalArrows) this._$modalArrows.css("display", "none"); //hide the arrows when showing video
    ...
    _preloadImage(src, $containerForImage) {
      ...
      if ($containerForImage) {
        ...
        img.onload = () => {
          ...
          if (this._$modalArrows) this._$modalArrows.css("display", ""); // remove display to default to css property
