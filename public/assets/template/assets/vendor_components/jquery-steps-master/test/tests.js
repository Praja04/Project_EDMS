...
test("contentMode", 5, function () {
  ...
  var contentModeWithNumberArgument = $(
    "#contentModeWithNumberArgument",
  ).steps();
  equal(
    contentModeWithNumberArgument.steps("getCurrentStep").contentMode,
    0,
    "Valid number argument",
  );

  var contentModeWithStringArgument = $(
    "#contentModeWithStringArgument",
  ).steps();
  equal(
    contentModeWithStringArgument.steps("getCurrentStep").contentMode,
    0,
...
test("stepClassFirstAndLast", 12, function () {
  ...
  ok(
    checkOnlyFirstItemHasClass(),
    "Valid after insert on first position (first)!",
  );
  ok(
    checkOnlyLastItemHasClass(),
    "Valid after insert on first position (last)!",
  );
...
test("stepClassCurrent", 6, function () {
  ...
  ok(
    checkOnlyItemOnPositionHasClass(2),
    "Valid after insert on first position!",
  );
...
test("stepClassDisabledAndDone", 12, function () {
  function checkOnlyItemAfterPositionHasClass(index) {
    ...
    return index > steps.length - 1
      ? $("#vis li[role=tab]:gt(" + index + ")").hasClass("disabled")
      : true;
  }
  ...
  function checkOnlyItemBeforePositionHasClass(index) {
    ...
    return index > 0
      ? $("#vis li[role=tab]:lt(" + index + ")").hasClass("done")
      : true;
  }
  ...
  ok(
    checkOnlyItemAfterPositionHasClass(2),
    "Valid after insert on first position (disabled)!",
  );
  ok(
    checkOnlyItemBeforePositionHasClass(2),
    "Valid after insert on first position (done)!",
  );
  ...
  $("#vis").steps("remove", 0);
  ok(
    checkOnlyItemAfterPositionHasClass(1),
    "Valid after remove first item (disabled)!",
  );
  ok(
    checkOnlyItemBeforePositionHasClass(1),
    "Valid after remove first item (done)!",
  );
...
test("stepCache", 4, function () {
  ...
  equal(
    getStep(wizard, 0).title,
    "insert",
    "Valid position after insert step to cache!",
  );
...
test("uniqueId", 5, function () {
  ...
  equal(
    wizardId2.substring(0, 10),
    "steps-uid-",
    "Valid auto id after initialization!",
  );
