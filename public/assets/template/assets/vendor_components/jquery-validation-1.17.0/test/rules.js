...
QUnit.test(
  "rules(), merge min/max to range, minlength/maxlength to rangelength",
  function (assert) {
...
QUnit.test("rules(), class and attribute combinations", function (assert) {
  ...
  assert.deepEqual($("#v2-i6").rules(), {
    required: true,
    customMethod2: true,
    rangelength: [2, 5],
  });
  jQuery.validator.autoCreateRanges = false;
  assert.deepEqual($("#v2-i7").rules(), {
    required: true,
    minlength: 2,
    customMethod: true,
  });
...
QUnit.test("rules(), add and remove", function (assert) {
  ...
  $("#v2-i5")
    .removeClass("required email")
    .removeAttr("minlength maxlength customMethod1 min");
  assert.deepEqual($("#v2-i5").rules(), {});
...
QUnit.test("rules(), global/local normalizer", function (assert) {
  ...
  v = $("#testForm1clean").validate({
    ...
    normalizer: function (value) {
      ...
      assert.notEqual(
        this,
        urlc[0],
        "This normalizer should not be called for urlc element.",
      );
      assert.equal(
        this,
        username[0],
        "`this` in this normalizer should be the username element.",
      );
      ...
      urlc: {
        ...
        normalizer: function (value) {
          assert.equal(
            this,
            urlc[0],
            "`this` in the normalizer should be the urlc element.",
          );
      ...
      lastname: {
        ...
        normalizer: function (value) {
          assert.equal(
            this,
            lastname[0],
            "`this` in the normalizer should be the lastname element.",
          );
  ...
  assert.equal(
    v.invalidElements()[0],
    username[0],
    "The username should be invalid",
  );
  ...
  assert.throws(
    ...
    function (err) {
      return (
        err.name === "TypeError" &&
        err.message === "The normalizer should return a string value."
      );
    },
...
QUnit.test("rules() - on unexpected input", function (assert) {
  ...
  assert.deepEqual(
    result,
    undefined,
    "can work on an empty set without a js error",
  );
