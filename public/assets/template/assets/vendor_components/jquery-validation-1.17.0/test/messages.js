...
QUnit.test(
  "predefined message not overwritten by addMethod( a, b, undefined )",
  function (assert) {
...
QUnit.test("group error messages", function (assert) {
  ...
  assert.equal(
    form.find(".errorContainer .error:not(input)").text(),
...
QUnit.test("read messages from metadata", function (assert) {
  ...
  e.valid();
  assert.equal(
    form.find("#testEmail9").next(".error:not(input)").text(),
    "required",
  );
  e.val("bla").valid();
  assert.equal(
  ...
  g.valid();
  assert.equal(
    form.find("#testGeneric9").next(".error:not(input)").text(),
    "generic",
  );
  g.val("bla").valid();
  assert.equal(
...
QUnit.test(
  "read messages from metadata, with meta option specified, but no metadata in there",
  function (assert) {
