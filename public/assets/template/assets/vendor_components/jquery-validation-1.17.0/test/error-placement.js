...
QUnit.test("error containers, simple", function (assert) {
        ...
        container
          ...
          .html(
            jQuery.validator.format(
              "There are {0} errors in your form.",
              this.size(),
            ),
          );
  ...
  assert.equal(
    container.find(".error:not(input)").length,
    0,
    "There should be no error labels",
  );
  ...
  assert.equal(
    container.find(".error:not(input)").length,
    2,
    "There should be two error labels",
  );
  ...
  assert.equal(
    container.find(".error:not(input)").length,
    2,
    "There should still be two error labels",
  );
...
QUnit.test("error containers, with labelcontainer I", function (assert) {
  ...
  assert.equal(
    container.find(".error:not(input)").length,
    0,
    "There should be no error labels in the container",
  );
  assert.equal(
    labelcontainer.find(".error:not(input)").length,
    0,
    "There should be no error labels in the labelcontainer",
  );
  assert.equal(
    labelcontainer.find("li").length,
    0,
    "There should be no lis labels in the labelcontainer",
  );
  ...
  assert.equal(
    container.find(".error:not(input)").length,
    0,
    "There should be no error label in the container",
  );
  assert.equal(
    labelcontainer.find(".error:not(input)").length,
    2,
    "There should be two error labels in the labelcontainer",
  );
  assert.equal(
    labelcontainer.find("li").length,
    2,
    "There should be two error lis in the labelcontainer",
  );
  assert.ok(container.is(":visible"), "Check that the container is visible");
  assert.ok(
    labelcontainer.is(":visible"),
    "Check that the labelcontainer is visible",
  );
  labelcontainer.find(".error:not(input)").each(function () {
    assert.ok($(this).is(":visible"), "Check that each label is visible1");
    assert.equal(
      $(this).parent()[0].tagName.toLowerCase(),
      "li",
      "Check that each label is wrapped in an li",
    );
    assert.ok(
      $(this).parent("li").is(":visible"),
      "Check that each parent li is visible",
    );
  });
});
...
QUnit.test("errorcontainer, show/hide only on submit", function (assert) {
  ...
  assert.ok(
    v.form(),
    "Form now valid, trigger showErrors but not invalid-form",
  );
});
...
QUnit.test("test label used as error container", function (assert) {
  ...
  assert.equal(
    field.next("label").contents().first().text(),
    "Field Label",
    "container label isn't disrupted",
  );
  assert.hasError(field, "missing");
  assert.ok(
    !field.attr("aria-describedby"),
    "field does not require aria-describedby attribute",
  );
...
QUnit.test(
  "test error placed adjacent to descriptive label",
  function (assert) {
    assert.expect(8);
    var form = $("#testForm16"),
      field = $("#testForm16text");
...
QUnit.test(
  "test descriptive label used alongside error label",
  function (assert) {
    assert.expect(8);
    var form = $("#testForm16"),
      field = $("#testForm16text");
...
QUnit.test(
  "test aria-describedby with input names contains CSS-selector meta-characters",
  function (assert) {
    var form = $("#testForm21"),
      field = $(
        "#testForm21\\!\\#\\$\\%\\&\\'\\(\\)\\*\\+\\,\\.\\/\\:\\;\\<\\=\\>\\?\\@\\[\\\\\\]\\^\\`\\{\\|\\}\\~",
...
QUnit.test("test existing non-error aria-describedby", function (assert) {
  ...
  assert.strictEqual(
    $("#testForm17text-description").text(),
    "This is where you enter your data",
  );
  assert.strictEqual(
    $("#testForm17text-error").text(),
    "",
    "Error label is empty for valid field",
  );
});
...
QUnit.test("test id/name containing single quotes", function (assert) {
  ...
  assert.equal(
    v.invalidElements()[0],
    textElement[0],
    "The element should be invalid",
  );
  assert.equal(
    v.invalidElements()[1],
    checkboxElement[0],
    "The text element should be invalid",
  );
  assert.equal(
    v.invalidElements()[2],
    radioElement[0],
...
QUnit.test(
  "#1632: Error hidden, but input error class not removed",
  function (assert) {
    ...
    assert.equal(
      v.invalidElements()[0],
      box2[0],
      "The box2 element should be invalid",
    );
    ...
    assert.equal(
      box2.hasClass("error"),
      false,
      "Box2 should not have an error class",
    );
  },
);
