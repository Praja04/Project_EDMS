...
QUnit.test("url", function (assert) {
  ...
  assert.ok(
    method("http://bassistance.de/jquery/plugin.php?bla=blu"),
    "Valid url",
  );
  assert.ok(
    method("https://bassistance.de/jquery/plugin.php?bla=blu"),
    "Valid url",
  );
  assert.ok(
    method("ftp://bassistance.de/jquery/plugin.php?bla=blu"),
    "Valid url",
  );
  assert.ok(
    method("http://www.føtex.dk/"),
    "Valid url, danish unicode characters",
  );
  assert.ok(
    method("http://bösendorfer.de/"),
    "Valid url, german unicode characters",
  );
  assert.ok(method("http://142.42.1.1"), "Valid IP Address");
  assert.ok(method("http://pro.photography"), "Valid long TLD");
  assert.ok(
    method("//code.jquery.com/jquery-1.11.3.min.js"),
    "Valid protocol-relative url",
  );
  assert.ok(method("//142.42.1.1"), "Valid protocol-relative IP Address");
  assert.ok(
    !method("htp://code.jquery.com/jquery-1.11.3.min.js"),
    "Invalid protocol",
  );
...
QUnit.test("required with dependencies", function (assert) {
  ...
  assert.ok(
    method.call(v, e[1].value, e[1], "asfsfa"),
    "Valid select due to dependency not met",
  );
  assert.ok(!method.call(v, e[1].value, e[1], "input"), "Invalid select");
  assert.ok(
    method.call(v, e[2].value, e[2], "asfsafsfa"),
    "Valid textarea due to dependency not met",
  );
  assert.ok(!method.call(v, e[2].value, e[2], "input"), "Invalid textarea");
  assert.ok(
    method.call(v, e[3].value, e[3], "asfsafsfa"),
    "Valid radio due to dependency not met",
  );
  assert.ok(!method.call(v, e[3].value, e[3], "input"), "Invalid radio");
  assert.ok(
    method.call(v, e[4].value, e[4], "asfsafsfa"),
    "Valid checkbox due to dependency not met",
  );
...
QUnit.test("minlength", function (assert) {
  ...
  assert.ok(
    !method.call(v, e[1].value, e[1], param),
    "Invalid select " + e[1].id,
  );
...
QUnit.test("#1760 - step modulo/remainder regression tests", function (assert) {
  ...
  for (var i = 1; i <= 1000; i++) {
    ...
    assert.ok(
      method.call(v, e[0].value, e[0], param),
      "Ensure " + e[0].value + " % " + param + " === 0 is valid",
    );
  }
});
...
QUnit.test("extension", function (assert) {
  ...
  assert.ok(
    method("picture.pdf", "doc,pdf"),
    "Valid custom accept type, comma separated",
  );
  assert.ok(
    method("picture.pdf", "pdf,doc"),
    "Valid custom accept type, comma separated",
  );
  assert.ok(
    !method("picture.pdf", "gop,top"),
    "Invalid custom accept type, comma separated",
  );
});
...
QUnit.test("remote", function (assert) {
  ...
  var e = $("#username"),
    v = $("#userForm").validate({
      ...
      submitHandler: function () {
        assert.ok(
          false,
          "submitHandler may never be called when validating only elements",
        );
      },
    }),
    done = assert.async();
  ...
  $(document).ajaxStop(function () {
    ...
    assert.strictEqual(
      v.element(e),
      true,
      "new value, new request; dependency-mismatch considered as valid though",
    );
  });
  assert.strictEqual(
    v.element(e),
    false,
    "invalid element, nothing entered yet",
  );
  e.val("Peter");
  assert.strictEqual(
    v.element(e),
    true,
    "still invalid, because remote validation must block until it returns; dependency-mismatch considered as valid though",
  );
});

QUnit.test(
  "remote, pending class added to element while call outstanding",
  function (assert) {
    assert.expect(3);
    var e = $("#username"),
      done = assert.async(),
            ...
            remote: {
              ...
              complete: function () {
                assert.strictEqual(
                  e.hasClass("pending"),
                  false,
                  "not pending since ajax call complete",
                );
                done();
              },
            },
    ...
    assert.strictEqual(
      e.hasClass("pending"),
      false,
      "not pending since no data entered",
    );
    e.val("Peter");
    ...
    assert.strictEqual(
      e.hasClass("pending"),
      true,
      "pending while validation outstanding",
    );
  },
);
...
QUnit.test("remote extensions", function (assert) {
  ...
  $(document).ajaxStop(function () {
    ...
    if (v.size() !== 0) {
      ...
      assert.equal(
        v.errorList[0].message,
        "asdf is already taken, please try something else",
      );
      v.element(e);
      assert.equal(
        v.errorList[0].message,
        "asdf is already taken, please try something else",
        "message doesn't change on revalidation",
      );
    }
    done();
  });
...
QUnit.test("remote, data previous querystring", function (assert) {
  ...
  assert.strictEqual(
    succeeded,
    0,
    "no valid call means no successful validation",
  );
  v.element($l);
  setTimeout(function () {
    assert.strictEqual(
      succeeded,
      1,
      "first valid check should submit given first name",
    );
    done1();
    v.element($l);
    setTimeout(function () {
      assert.strictEqual(
        succeeded,
        1,
        "second valid check should not resubmit given same first name",
      );
      done2();
      ...
      setTimeout(function () {
        assert.strictEqual(
          succeeded,
          2,
          "third valid check should resubmit given different first name",
        );
        done3();
      });
    });
...
QUnit.test("remote, highlight all invalid fields", function (assert) {
  ...
  setTimeout(function () {
    assert.equal(
      $firstnameField.hasClass("error"),
      true,
      "Field 'firstname' should have a '.error' class",
    );
    assert.equal(
      $lastnameField.hasClass("error"),
      true,
      "Field 'lastname' should have a '.error' class",
    );
    assert.equal(
      $somethingField.hasClass("error"),
      true,
      "Field 'something' should have a '.error' class",
    );
    done();
  }, 500);
});
QUnit.test(
  "remote, unhighlighted should be invoked after being highlighted/invalid",
  function (assert) {
    assert.expect(6);
    ...
    setTimeout(function () {
      assert.equal(
        $somethingField.hasClass("error"),
        true,
        "Field 'something' should have the error class",
      );
      done1();
      ...
      setTimeout(function () {
        assert.equal(
          $somethingField.hasClass("error"),
          false,
          "Field 'something' should not have the error class",
        );
        done2();
      }, 500);
    }, 500);
...
QUnit.test(
  "Fix #697: remote validation uses wrong error messages",
  function (assert) {
    var e = $("#username"),
      done1 = assert.async(),
      done2 = assert.async(),
...
QUnit.test("phone (us)", function (assert) {
  ...
  assert.ok(
    !method("111-123-5434"),
    "Invalid US phone number. Area Code cannot start with 1",
  );
  assert.ok(
    !method("212 123 4567"),
    "Invalid US phone number. NXX cannot start with 1",
  );
  assert.ok(
    !method("234-911-5678"),
    "Invalid US phone number, because the exchange code cannot be in the form N11",
  );
  assert.ok(
    !method("911-333-5678"),
    "Invalid US phone number, because the area code cannot be in the form N11",
  );
...
QUnit.test("iban", function (assert) {
  ...
  assert.ok(
    method("XX40INGB000123456712341234"),
    "Valid (more or less) IBAN: unknown country, but checksum OK",
  );
  ...
  assert.ok(
    !method("DE68 2105 0170 0012 3456 79"),
    "Invalid IBAN: checksum incorrect",
  );

  assert.ok(
    !method("NL54INGB00012345671234"),
    "Invalid IBAN too long, BUT CORRECT CHECKSUM",
  );
  assert.ok(
    !method("XX00INGB000123456712341234"),
    "Invalid IBAN: unknown country and checksum incorrect",
  );
  ...
  assert.ok(
    method("MT84 MALT 0110 0001 2345 MTLC AST0 01S"),
    "Valid IBAN - MT",
  );
...
QUnit.test("phoneNL", function (assert) {
  ...
  assert.ok(
    !method("+310201234567"),
    "Invalid phone NL: no 0 after +31 allowed",
  );
});
...
QUnit.test("mobileNL", function (assert) {
  ...
  assert.ok(
    !method("0123456789"),
    "Invalid NL Mobile Number: should start with 06",
  );
  assert.ok(
    !method("0823456789"),
    "Invalid NL Mobile Number: should start with 06",
  ...
  assert.ok(
    !method("+310612345678"),
    "Invalid NL Mobile Number: no 0 after +31 allowed",
  );
});
...
QUnit.test("bankaccountNL", function (assert) {
  ...
  assert.ok(
    !method("12 3456789"),
    "Valid NL bank account: inconsistent spaces",
  );
...
QUnit.test("rangeWords", function (assert) {
  ...
  assert.ok(
    !method("<b>Hello</b> <b>world</b> <b>hello</b>", [0, 2]),
    "html, invalid",
  );
});
...
QUnit.test("pattern", function (assert) {
  ...
  assert.ok(
    method("AR1004", "AR\\d{4}"),
    "Correct format for the given RegExp",
  );
  assert.ok(
    method("AR1004", /^AR\d{4}$/),
    "Correct format for the given RegExp",
  ...
  assert.ok(
    !method("BR1004", /^AR\d{4}$/),
    "Invalid format for the given RegExp",
  );
  assert.ok(
    method("1ABC", "[0-9][A-Z]{3}"),
    "Correct format for the given RegExp",
  );
  assert.ok(
    !method("ABC", "[0-9][A-Z]{3}"),
    "Invalid format for the given RegExp",
  );
  assert.ok(
    !method("1ABC DEF", "[0-9][A-Z]{3}"),
    "Invalid format for the given RegExp",
  );
  assert.ok(
    method("1ABCdef", "[a-zA-Z0-9]+"),
    "Correct format for the given RegExp",
  );
  assert.ok(
    !method("1ABC def", "[a-zA-Z0-9]+"),
    "Invalid format for the given RegExp",
  );
  assert.ok(
    method(
...
function testCardTypeByNumber(assert, number, cardname, expected) {
  ...
  assert.equal(
    ...
    expected,
    $.validator.format(
      "Expect card number {0} to validate to {1}, actually validated to ",
      number,
      expected,
    ),
  );
...
function fillFormWithValuesAndExpect(
  ...
  assert.equal(
    ...
    expected,
    $.validator.format(
      "Filled inputs of form '{0}' with {1} values ({2})",
      formSelector,
      inputValues.length,
...
QUnit.test("nifES", function (assert) {
  ...
  assert.ok(
    !method("1144105R"),
    "NIF invalid: less than 8 digits without zero",
  );
  ...
  assert.ok(
    !method("05.122.654-R"),
    "NIF invalid: starts with zero and dot and dash",
  );
});
...
QUnit.test("nieES", function (assert) {
  ...
  assert.ok(
    !method("05.122.654-R"),
    "NIE invalid: starts with zero and dot and dash",
  );
});
...
QUnit.test("cifES", function (assert) {
  ...
  assert.ok(
    !method("A7908224D"),
    "CIF invalid: digit control must be a number (4)",
  );
  ...
  assert.ok(
    !method("B8352471J"),
    "CIF invalid: digit control must be a number (0)",
  );
  ...
  assert.ok(
    method("C2782755I"),
    "CIF valid. Digit control can be either a number or letter",
  );
  assert.ok(
    method("C27827559"),
    "CIF valid. Digit control can be either a number or letter",
  ...
  assert.ok(
    !method("E1747295B"),
    "CIF invalid: digit control must be a number (2)",
  );
  ...
  assert.ok(
    !method("K39022389"),
    "CIF invalid. Digit control must be a letter (I)",
  );
  ...
  assert.ok(
    !method("P98038813"),
    "CIF invalid: digit control must be a letter (C)",
  );
...
QUnit.test("maxWords", function (assert) {
  ...
  assert.ok(
    !method("I'm way too long for this sentence!", maxWords),
    "Too many words",
  );
  assert.ok(
    method("Don’t “count” me as too long", maxWords),
    "Right amount of words with smartquotes",
  );
  assert.ok(
    !method("But you can “count” me as too long", maxWords),
    "Too many words with smartquotes",
  );
  assert.ok(
    method("<div>Don’t “count” me as too long</div>", maxWords),
    "Right amount of words with smartquotes w/ HTML",
  );
  assert.ok(
    !method("<div>But you can “count” me as too long</div>", maxWords),
    "Too many words with smartquotes w/ HTML",
  );
});
...
QUnit.test("minWords", function (assert) {
  ...
  assert.ok(
    method("I'm way too long for this sentence!", minWords),
    "Too many words",
  );
  assert.ok(
    !method("Don’t “count” me as short.", minWords),
    "Right amount of words with smartquotes",
  );
  assert.ok(
    method("But you can “count” me as too short", minWords),
    "Too many words with smartquotes",
  );
  assert.ok(
    !method("<div>“Count” me as too short.</div>", minWords),
    "Right amount of words with smartquotes w/ HTML",
  );
  assert.ok(
    method("<div>But you can “count” me as too long</div>", minWords),
    "Too many words with smartquotes w/ HTML",
  );
});
...
QUnit.test("rangeWords", function (assert) {
  ...
  assert.ok(
    !method("I'm going to be longer than “six words!”", rangeWords),
    "Longer than 6 with smartquotes",
  );
  ...
  assert.ok(
    method("<div>“Count” me as perfect.</div>", rangeWords),
    "Right amount of words with smartquotes w/ HTML",
  );
  assert.ok(
    !method("<div>But you can “count” me as too long</div>", rangeWords),
    "Too many words with smartquotes w/ HTML",
  );
});
...
QUnit.test("currency", function (assert) {
  ...
  assert.ok(
    method("£9,999.9", "£"),
    "£, thousand, comma separator, one decimal",
  );
  assert.ok(
    method("£9,999.99", "£"),
    "£, thousand, comma separator, two decimal",
  );
  assert.ok(
    method("£9,999,999.9", "£"),
    "£, million, comma separators, one decimal",
  );
...
QUnit.test("postalCodeCA", function (assert) {
  ...
  assert.ok(
    method("H0H0H0"),
    "Valid Canadian postal code: all upper case with no space",
  );
  assert.ok(
    method("H0H 0H0"),
    "Valid Canadian postal code: all upper case with one space",
  );
  assert.ok(
    method("H0H  0H0"),
    "Valid Canadian postal code: all upper case with multiple spaces",
  );
  assert.ok(
    method("h0h 0h0"),
    "Valid Canadian postal code: all lower case with space",
  );
  assert.ok(
    method("h0h0h0"),
    "Valid Canadian postal code: all lower case with no space",
  );
  assert.ok(
    !method("H0H-0H0"),
    "Invalid Canadian postal code: dash used as separator",
  );
  assert.ok(!method("H0H 0H"), "Invalid Canadian postal code: too short");
  assert.ok(
    !method("Z0H 0H0"),
    "Invalid Canadian postal code: only 'ABCEGHJKLMNPRSTVXY' are valid first characters for the Forward Sorting Area",
  );
  assert.ok(
    !method("H0D 0H0"),
    "Invalid Canadian postal code: only 'ABCEGHJKLMNPRSTVWXYZ' are valid third characters for the Forward Sorting Area",
  );
  assert.ok(
    !method("H0H 0D0"),
    "Invalid Canadian postal code: only 'ABCEGHJKLMNPRSTVWXYZ' are valid second characters for the Local Delivery Unit",
  );
});
...
QUnit.test("stateUS", function (assert) {
  ...
  assert.ok(
    !method("mp", { caseSensitive: false, includeTerritories: false }),
    "US territories not allowed",
  );
});
...
QUnit.test("cpfBR", function (assert) {
  ...
  assert.ok(
    !method("11144477715"),
    "Invalid CPF Number: 1st check number failed",
  );
  assert.ok(
    !method("11144477737"),
    "Invalid CPF Number: 2nd check number failed",
  );
});
...
QUnit.test("file accept - image wildcard", function (assert) {
  var input = acceptFileDummyInput("test.png", "image/png"),
    $form = $("<form />"),
    proxy = $.proxy(
      $.validator.methods.accept,
      new $.validator({}, $form[0]),
      null,
      input,
      "image/*",
    );
...
QUnit.test("file accept - specified mime type", function (assert) {
  var input = acceptFileDummyInput(
      "test.kml",
...
QUnit.test("file accept - invalid mime type", function (assert) {
  var input = acceptFileDummyInput(
      "test.kml",
      "foobar/vnd.google-earth.kml+xml",
    ),
    $form = $("<form />"),
    proxy = $.proxy(
      $.validator.methods.accept,
  ...
  assert.equal(
    proxy(),
    false,
    "the selected file for upload has invalid mime type",
  );
});
