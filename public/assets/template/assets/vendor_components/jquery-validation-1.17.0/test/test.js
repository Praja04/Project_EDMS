...
$.mockjax({
  ...
  response: function (data) {
    ...
    if (this.responseStatus === 200) {
      //Only if valid request - processing workemail
      this.responseText = JSON.stringify(
        data.data.email === "john.doe@company.com",
      );
    }
  },
});
...
QUnit.assert.noErrorFor = function (element, message) {
  var errors = $(element).closest("form").validate().errorsFor(element[0]),
    hidden =
      errors.length === 0 || (errors.is(":hidden") && errors.text() === "");
  this.pushResult({
    result: hidden,
    actual: hidden,
    expected: true,
...
QUnit.test(
  "validate() without elements, with non-form elements",
  function (assert) {
    assert.expect(0);
...
QUnit.test(
  "valid() plugin method, special handling for checkable groups",
  function (assert) {
...
QUnit.test("addMethod", function (assert) {
  ...
  assert.ok(method(e.value, e), "Invalid");
  assert.ok(
    jQuery.validator.messages.hi === "hi me too",
...
QUnit.test(
  "numberOfInvalids(): count invalid fields with empty message",
  function (assert) {
    assert.expect(2);
...
QUnit.test(
  "submitHandler keeps submitting button, even if descendants are clicked",
  function (assert) {
    ...
    var v = $("#testForm27").validate({
      ...
      submitHandler: function (form) {
        ...
        assert.deepEqual(
          v.submitButton,
          button,
          "The submitButton property should be the same as button",
        );
...
QUnit.test(
  "handle() should ensure the value of the used submit button is passed on for scripted submit triggered by stopRequest()",
  function (assert) {
    var $form = $("#userForm");
...
QUnit.test(
  "validation triggered on radio/checkbox when using keyboard",
  function (assert) {
    assert.expect(1);
    var input,
      i,
    ...
    events = [$.Event("focusin"), $.Event("focusout"), $.Event("keyup")];
...
QUnit.test(
  "validation triggered on radio/checkbox when using mouseclick",
  function (assert) {
    assert.expect(1);
    var input,
      i,
...
QUnit.test(
  "showErrors(), allow empty string and null as default message",
  function (assert) {
    $("#userForm").validate({
      rules: {
        username: {
  ...
  var v = $("#testForm1").validate({
    errorPlacement: function () {
      assert.strictEqual(
        this,
        v,
        "'this' inside errorPlacement should be the plugin instance",
      );
    },
  });
...
QUnit.test("defaultMessage(), empty title is ignored", function (assert) {
  ...
  assert.equal(
    v.defaultMessage($("#username")[0], "required"),
    "This field is required.",
  );
});
...
QUnit.test(
  "#741: move message processing from formatAndAdd to defaultMessage",
  function (assert) {
    var v = $("#testForm22").validate();
    assert.equal(
      v.defaultMessage($("#tF22Input")[0], {
        method: "minlength",
        parameters: 5,
      }),
      "You should enter at least 5 characters.",
      "defaultMessage() now format the messages",
    );
...
QUnit.test(
  "focusInvalid() after validate a custom set of inputs",
  function (assert) {
    var form = $("#testForm1"),
      validator = form.validate(),
    ...
    assert.equal(
      form[0].ownerDocument.activeElement,
      $("#lastname")[0],
...
QUnit.test(
  "elementValue() finds radios/checkboxes only within the current form",
  function (assert) {
    assert.expect(1);
    var v = $("#userForm").validate(),
      foreignRadio = $("#radio2")[0];
...
QUnit.test(
  "elementValue() returns the file input's name without the prefix 'C:\\fakepath\\' ",
  function (assert) {
    var v = $("#userForm").validate(),
      // A fake file input
      fileInput = {
    ...
    assert.ok(
      v.element(fileInput),
      "The fake file input is valid (length = 12, minlength = 10)",
    );

    fileInput.value = "C:\\fakepath\\file.txt";
    assert.ok(
      !v.element(fileInput),
      "The fake file input is invalid (length = 8, minlength = 10)",
    );
    ...
    assert.ok(
      v.element(fileInput),
      "The fake file input is valid (length = 8, maxlength = 10)",
    );

    fileInput.value = "C:\\fakepath\\fakefile.txt";
    assert.ok(
      !v.element(fileInput),
      "The fake file input is invalid (length = 12, maxlength = 10)",
    );
  },
);
...
QUnit.test("", function (assert) {
  ...
  assert.equal(
    v.errorList[0].message,
    $.validator.messages.required,
    "The error message should be the one of required rule.",
  );
  ...
  assert.equal(
    ...
    "The error message should be the one of number rule.",
  );
...
QUnit.test("dynamic form", function (assert) {
  ...
  function add() {
    $(
      "<input data-rule-required='true' name='list" + counter++ + "' />",
    ).appendTo("#testForm2");
  }
...
QUnit.test("resetForm()", function (assert) {
  ...
  assert.equal(
    $("#firstname").val(),
    "",
    "form plugin is included, therefor resetForm must also reset inputs, not only errors",
  );
});

QUnit.test(
  "resetForm() clean styles when custom highlight function is used",
  function (assert) {
...
QUnit.test("ignoreTitle", function (assert) {
  ...
  assert.equal(
    v.errorList[0].message,
    $.validator.messages.required,
    "title used when it should have been ignored",
  );
});
...
QUnit.test("bypassing validation on form submission", function (assert) {
  var form = $("#bypassValidation"),
    ...
    bypassSubmitWithCancel = $(
      "form#bypassValidation :input[id=bypassSubmitWithCancel]",
    ),
    bypassSubmitWithNoValidate1 = $(
      "form#bypassValidation :input[id=bypassSubmitWithNoValidate1]",
    ),
    bypassSubmitWithNoValidate2 = $(
      "form#bypassValidation :input[id=bypassSubmitWithNoValidate2]",
    ),
    $v = form.validate({
      debug: true,
    });
  ...
  assert.equal(
    $v.numberOfInvalids(),
    0,
    "Validation was bypassed using CSS 'cancel' class.",
  );
  $v.resetForm();
  ...
  assert.equal(
    $v.numberOfInvalids(),
    0,
    "Validation was bypassed using blank 'formnovalidate' attribute.",
  );
  $v.resetForm();
  ...
  assert.equal(
    $v.numberOfInvalids(),
    0,
    "Validation was bypassed using 'formnovalidate=\"formnovalidate\"' attribute.",
  );
  $v.resetForm();
...
QUnit.test("works on contenteditable fields", function (assert) {
  ...
  form.valid();
  assert.hasError(
    $("#contenteditableNumberInvalid"),
    "Please enter a valid number.",
  );
  assert.hasError(
    $("#contenteditableRequiredInvalid"),
    "This field is required.",
  );
...
QUnit.test(
  "Assign rules to contenteditable via .validate() method",
  function (assert) {
    assert.expect(2);
...
QUnit.test(
  "Assign rules to contenteditable via .rules() method",
  function (assert) {
    assert.expect(2);
    var form = $("#_contenteditableForm");
    var v = form.validate({
...
QUnit.test(
  "success isn't called for optional elements with no other rules",
  function (assert) {
    assert.expect(4);
    assert.equal(
      $("#firstname")
        .removeAttr("data-rule-required")
        .removeAttr("data-rule-minlength")
        .val(),
      "",
    );
    $("#something").remove();
...
QUnit.test(
  "success is called for optional elements with other rules",
  function (assert) {
    assert.expect(1);
...
QUnit.test(
  "all rules are evaluated even if one returns a dependency-mistmatch",
  function (assert) {
    assert.expect(6);
    assert.equal(
      $("#firstname")
...
QUnit.test("jQuery.validator.format", function (assert) {
  ...
  assert.equal(
    jQuery.validator.format(
      "{0} is too fast! Enter a value smaller then {0} and at least {1}",
      0,
      -15,
    ),
  ...
  template = jQuery.validator.format(
    "Please enter a value between {0} and {1}.",
  );
...
QUnit.test("validate on blur", function (assert) {
  ...
  errors(
    1,
    "Required was ignored, but as something was entered, check other rules, minlength isn't met",
  );
  labels(1);
  e.val("hh");
  blur(e);
  ...
  errors(
    1,
    "Blurring the field results in emptying the error list first, then checking the invalid field: its still invalid, don't remove the error",
  );
  labels(3);
  e.val("h");
  blur(e);
  errors(
    1,
    "Entering a single character fulfills required, but not minlength: 2, still invalid",
  );
  labels(3);
  e.val("hh");
  blur(e);
...
QUnit.test(
  "don't revalidate the field when pressing special characters",
  function (assert) {
    function errors(expected, message) {
      assert.equal(v.size(), expected, message);
    }
...
QUnit.test(
  "#1508: Validation fails to trigger when next field is already filled out",
  function (assert) {
    ...
    var event = $.Event("keyup", { keyCode: 9 }),
      ...
      eq = function (expected, msg) {
        assert.equal(
          ...
          expected,
          "There is only one invalid element.",
        );
...
QUnit.test(
  "[Remote rule] #1508: Validation fails to trigger when next field is already filled out",
  function (assert) {
    assert.expect(2);
      ...
      check = function (value) {
        ...
        setTimeout(function () {
          ...
          setTimeout(function () {
            ...
            assert.equal(
              validator.invalidElements()[0],
              element[0],
              "The val2 element should be invalid",
            );
            done();
          });
        });
...
QUnit.test("correct checkbox receives the error", function (assert) {
  ...
  assert.equal(v.form(), false);
  assert.ok(
    v.errorList[0].element.id === v.currentElements[0].id,
    "the proper checkbox has the error AND is present in currentElements",
  );
});
...
QUnit.test(
  "validate input with no type attribute, defaulting to text",
  function (assert) {
    function errors(expected, message) {
      assert.equal(v.size(), expected, message);
    }
...
QUnit.test("ignore hidden elements at start", function (assert) {
  ...
  assert.ok(
    !validate.form(),
    "form should be invalid when required element is visible",
  );
});
...
QUnit.test("Max date set by attribute", function (assert) {
  ...
  assert.equal(
    label.text(),
    "Please enter a value less than or equal to 2012-12-21.",
    "Correct error label",
  );
});
...
QUnit.test("Min and Max date set by attributes greater", function (assert) {
  ...
  assert.equal(
    label.text(),
    "Please enter a value less than or equal to 2013-01-21.",
    "Correct error label",
  );
});
...
QUnit.test("Min and Max strings set by attributes greater", function (assert) {
  ...
  assert.equal(
    label.text(),
    "Please enter a value less than or equal to 200.",
    "Correct error label",
  );
});
...
QUnit.test(
  "Min, Max and Step strings set by attributes valid",
  function (assert) {
    var form = $("#ranges"),
...
QUnit.test(
  "Step attribute on element with unsupported input type",
  function (assert) {
    ...
    assert.throws(
      ...
      function (err) {
        return (
          err.message === "Step attribute on input type date is not supported."
        );
      },
...
QUnit.test(
  "Min and Max type absent set by attributes greater",
  function (assert) {
    var form = $("#ranges"),
...
QUnit.test(
  "Min, Max and Step type absent set by attributes valid",
  function (assert) {
    var form = $("#ranges"),
...
QUnit.test(
  "Min, Max and Step range set by attributes valid",
  function (assert) {
    //
...
QUnit.test(
  "Rules allowed to have a value of zero valid equal",
  function (assert) {
    var form = $("#ranges"),
...
QUnit.test(
  "Rules allowed to have a value of zero valid greater",
  function (assert) {
    var form = $("#ranges"),
...
QUnit.test(
  "Validation triggered on radio and checkbox via click",
  function (assert) {
    assert.expect(2);
...
QUnit.test("destroy()", function (assert) {
  ...
  form.valid();
  assert.equal(
    $("#x1", form).hasClass("validate-equalTo-blur"),
    true,
    "The blur event should be bound to this element",
  );
  assert.equal(
    $("#x2", form).hasClass("validate-equalTo-blur"),
    true,
    "The blur event should be bound to this element",
  );
  ...
  assert.equal(
    $("#x1", form).hasClass("validate-equalTo-blur"),
    false,
    "The blur event should be unbound from this element",
  );
  assert.equal(
    $("#x2", form).hasClass("validate-equalTo-blur"),
    false,
...
QUnit.test(
  "#1618: Errorlist containing more errors than it should",
  function (assert) {
    ...
    assert.equal(
      v.errorList.length,
      2,
      "There should be no change in errorList's content",
    );
  },
);
...
QUnit.test("addMethod, reusing remote in custom method", function (assert) {
  ...
  $.validator.addMethod(
    ...
    function (value, element, param) {
      return $.validator.methods.remote.call(
        this,
        value,
        element,
        {
  ...
  $(document).ajaxStop(function () {
    ...
    assert.strictEqual(
      v.errorList[0].message,
      "work email custom message",
      "john.doe@gmail.com is not work email",
    );
