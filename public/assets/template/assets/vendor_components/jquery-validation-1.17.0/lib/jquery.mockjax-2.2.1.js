...
(function (root, factory) {
  ...
  function getQueryParams(queryString) {
    ...
    logger.debug(null, [
      "Getting query params from string",
      queryString,
      paramsObj,
    ]);
  ...
  function _xhrSend(mockHandler, requestSettings, origSettings) {
    logger.debug(mockHandler, [
      "Sending fake XHR request",
      mockHandler,
      requestSettings,
      origSettings,
    ]);
    ...
    if (mockHandler.proxy) {
      logger.info(mockHandler, [
        "Retrieving proxy file: " + mockHandler.proxy,
        mockHandler,
      ]);
  ...
  function processJsonpRequest(requestSettings, mockHandler, origSettings) {
    ...
    if (mockHandler.response && $.isFunction(mockHandler.response)) {
      ...
      logger.info(mockHandler, [
        "Performing JSONP proxy request: " + mockHandler.proxy,
        mockHandler,
      ]);
  ...
  function completeJsonpCall(
    ...
    setTimeout(function () {
      ...
      if (newMock) {
        ...
        logger.log(mockHandler, [
          "JSONP mock call complete",
          mockHandler,
          newMock,
        ]);
      }
  ...
  function createJsonpCallback(requestSettings, mockHandler, origSettings) {
    var callbackContext =
      (origSettings && origSettings.context) || requestSettings;
    var jsonp =
      (typeof requestSettings.jsonpCallback === "string" &&
        requestSettings.jsonpCallback) ||
      "jsonp" + jsc++;
  ...
  function jsonpSuccess(requestSettings, callbackContext, mockHandler) {
    ...
    if (requestSettings.global) {
      (requestSettings.context ? $(requestSettings.context) : $.event).trigger(
        "ajaxSuccess",
        [{}, requestSettings],
      );
    }
  ...
  function handleAjax(url, origSettings) {
    ...
    for (var k = 0; k < mockHandlers.length; k++) {
      ...
      if (!mockHandler) {
        logger.debug(mockHandlers[k], [
          "Mock does not match request",
          url,
          requestSettings,
        ]);
  ...
  var logger = {
    _log: function logger(mockHandler, args, level) {
      ...
      if ($.mockjaxSettings.log) {
        ...
        $.mockjaxSettings.logger[$.mockjaxSettings.logLevelMethods[level]]
      ) {
        return $.mockjaxSettings.logger[
          $.mockjaxSettings.logLevelMethods[level]
        ].apply($.mockjaxSettings.logger, args);
      }
    },
    /**
