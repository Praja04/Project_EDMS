...
(function ($) {
  ...
  function animationSettings($strip) {
    ...
    var distance = Math.abs(
      -$strip.css(settings.direction).replace("px", "").replace("auto", "0") -
  ...
  function initialize($strip, init) {
    ...
    if (settings.duplicate) {
      ...
      while (
        stripWidth - itemWidth < $strip.parent().width() ||
            ...
            function () {
              if (cssTransitionsSupported) {
                ...
                $(this)
                  ...
                  .css(settings.direction, currentPosition);
              } else {
                jQuery(this).stop();
              }
            },
          ...
          for (var i = list.length - 1; i >= 0; i--) {
            ...
            if (match.length < 1) {
              if (insert) {
                ...
                if (
                  $strip.find(".ticker-spacer:first-child").length === 0 &&
                  $strip.find(".ticker-spacer").length > 0
                ) {
