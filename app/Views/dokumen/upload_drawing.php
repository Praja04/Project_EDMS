                        ...
                        <div class="box-body pe-0 ps-lg-50 ps-15 py-0">
                            <div class="row align-items-center">
                                <div class="col-12 col-lg-8">
                                    <h1 class="fs-40 text-white">Upload Dokumen Drawing</h1>
                                        ...
                                        <tbody id="user">
                                            ...
                                            foreach ($dokumen as $user) : ?>
                                                <tr class="">
                                                    ...
                                                    <td>
                                                        <?php if ($user['file_path'] == null) : ?>
                                                            <button type="button" class="btn btn-primary upload-button" data-bs-toggle="modal" data-bs-target="#modal-right" data-id-pdf="<?= $user['dokumen_id'] ?>">
                                                                Upload Pdf
                                                            </button>
                                                        <?php else : ?>
                                                    ...
                                                    <td><?php if ($user['status'] === 'Verified') {
                                                            echo 'Masspro';
                                                        } else {
                                                            echo 'Not Masspro';
                                                        } ?></td>
                                                    <td>
                                                        <?php if ($user['file_path'] != null) : ?>
                                                            <a href="<?= base_url('uploader/pdf/setStatusMasspro/' . $user['dokumen_id']); ?>" class="btn btn-success" onclick="return confirm('Apakah Anda yakin ingin mengubah status menjadi masspro?')">Set Masspro</a>
                                                        <?php else : ?>
                                                            <p>Upload Dahulu !</p>
                                                        <?php endif; ?>
                                                    </td>
...
<script>
    $(document).ready(function() {
        ...
        $('#save-ulang-button').on('click', function() {
            ...
            $.ajax({
                url: baseUrl + 'uploader/pdf/update/' + $('#id-pdf').val(),
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false,
        ...
        $('#save-button').on('click', function() {
            ...
            $.ajax({
                url: baseUrl + 'uploader/pdf/upload',
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
