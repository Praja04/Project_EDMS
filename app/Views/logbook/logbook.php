...
<div class="content-wrapper">
    <div class="container-full">
        ...
        <section class="content">
                                                        ...
                                                        <tbody id="user2">
                                                            ...
                                                            <?php foreach ($data as $user) : ?>
                                                                ...
                                                                <tr class="text-center">
                                                                    ...
                                                                    <td>
                                                                        <?php if ($user['is_seen'] == '0') : ?>
                                                                            <p style="color:red;">
                                                                                <?= $user['pesan_pengajuan'] ?>
                                                                            </p> <br>
                                                                            <a class="btn btn-danger" data-bs-toggle="tooltip" data-bs-original-title="Delete Pengajuan" onclick="Delete_Pengajuan(<?= $user['pengajuan_id']; ?>)">
                                                                                <i class="mdi mdi-delete"></i>
                                                                            </a>
                                                                        <?php elseif ($user['is_seen'] == '1') : ?>
                                                                            -
                                                                        <?php elseif (is_null($user['is_seen'])) : ?>
                                                                            belum ada pengajuan
                                                                        <?php else : ?>
                                                                            -
                                                                        <?php endif; ?>
                                                                    ...
                                                                    <td style="display: flex; gap: 5px; ">
                                                                        <a class="btn btn-primary" data-bs-toggle="tooltip" data-bs-original-title="Lihat" href=" <?= base_url('admin/logbook/detail/' . $user['dokumen_id']) ?>">
                                                                            <i class="mdi mdi-eye"></i>
                                                                        </a>
                                                                        <a class="btn btn-danger" data-bs-toggle="tooltip" data-bs-original-title="Delete" onclick="Delete_number(<?= $user['dokumen_id']; ?>)">
                                                                            <i class="mdi mdi-delete"></i>
                                                                        </a>
                                                                        <?php if ($user['is_seen'] === 0) : ?>
                                                                            <button type="button" class="btn btn-link btn-info" data-bs-toggle="tooltip" data-bs-original-title="Pengajuan" onclick="PengajuanRevisi(<?php echo $user['dokumen_id']; ?>)" disabled>
                                                                                <i class="fa fa-commenting"></i>
                                                                            </button>
                                                                        <?php elseif (is_null($user['is_seen'])) : ?>
                                                                            <button type="button" class="btn btn-link btn-info" data-bs-toggle="tooltip" data-bs-original-title="Pengajuan" onclick="PengajuanRevisi(<?php echo $user['dokumen_id']; ?>)">
                                                                                <i class="fa fa-commenting"></i>
                                                                            </button>
                                                                        <?php elseif ($user['is_seen'] === 1) : ?>
                                                                            <button type="button" class="btn btn-link btn-info" data-bs-toggle="tooltip" data-bs-original-title="Pengajuan" onclick="PengajuanRevisi(<?php echo $user['dokumen_id']; ?>)">
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body text-center">
                            ...
                            <p>
                                Apakah Anda yakin ingin menghapus <strong>seluruh data dokumen</strong> ini?<br>
                                <small class="text-muted">(Tindakan ini juga akan menghapus semua file revisi terkait.)</small>
                            </p>
                        </div>
            ...
            <div class="modal fade" id="confirmDeletePengajuanModal" tabindex="-1" role="dialog" aria-labelledby="confirmDeleteModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="confirmDeleteModalLabel">Konfirmasi Hapus</h5>
                        ...
                        <div class="modal-body text-center">
                            <h5 class="mb-3">Konfirmasi Penghapusan</h5>
                            <p>
                                Apakah Anda yakin ingin menghapus <strong>pengajuan revisi</strong> ini?<br>
                            </p>
                        </div>

                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" id="batalDeletePengajuan" data-dismiss="modal">Batal</button>
                            <button type="button" class="btn btn-danger" id="confirmDeletePengajuanButton">Hapus</button>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-primary" id="confirmBtn">Ya, Ajukan Revisi</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    var idToDelete;
    var idtoDeletePengajuan;
    function Delete_number(idpdf) {
        idToDelete = idpdf;
        $('#confirmDeleteModal').modal('show');
    }
    function Delete_Pengajuan(idpdf) {
        idToDeletePengajuan = idpdf;
        $('#confirmDeletePengajuanModal').modal('show');
    }
    ...
    $(document).ready(function() {
        ...
        $('#batalDeletePengajuan').click(function() {
            $('#confirmDeletePengajuanModal').modal('hide');
        });

        $('#confirmDeleteButton').click(function() {
            $.ajax({
                url: '<?= base_url('admin/logbook/delete/') ?>' + idToDelete,
                type: 'DELETE',
        ...
        $('#confirmDeletePengajuanButton').click(function() {
            $.ajax({
                url: '<?= base_url('admin/logbook/delete/pengajuan/') ?>' + idToDeletePengajuan,
                type: 'DELETE',
                success: function(response) {
                    $('#confirmDeletePengajuanModal').modal('hide');
                    if (response.success === true) {
    ...
    function PengajuanRevisi(idpdf) {
        ...
        $('#confirmBtn').off('click').on('click', function() {
            ...
            $.ajax({
                url: baseUrl + 'admin/pengajuan/' + idpdf,
                type: 'POST',
                data: {
                    komentar: komentar
                },
